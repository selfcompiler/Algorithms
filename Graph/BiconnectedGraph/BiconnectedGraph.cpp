 ...
 *  0 articulation point s and all vertex should reachable from any vertex
 * */
//
...
bool findAP(int totalNodes){
    /*initialize all the arrays
     * */
    bool isAPexist=false;
...
bool isGraphConneted(int node,int totalNodes){
    for(int i=1;i<=totalNodes;i++)
        isVisited[i]=false;
    dfs(node);
    for(int i=1;i<=totalNodes;i++)
...
int main(){
    inScan();
    if(findAP(n)){
        puts("Graph is not Biconnected Articulation Point exits");
        exit(0);
    }
    if(!isGraphConneted(1,n)){
        puts("Graph is not connected");
        exit(0);
    }
    puts("Graph is Biconnected");
    return 0;
}
